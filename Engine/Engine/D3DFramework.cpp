...
D3DFramework::D3DFramework()
	...
	depthStencilView(nullptr), rasterState(nullptr) {}

D3DFramework::D3DFramework(const D3DFramework& other) {}

D3DFramework::~D3DFramework() {}

bool D3DFramework::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, 
						  float screenDepth, float screenNear)
{
	...
	if (FAILED(adapterOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM,
		DXGI_ENUM_MODES_INTERLACED, &numModes, NULL))) return false;
	...
	if (wcstombs_s(&stringLength, videoCardDescription, 128, adapterDesc.Description, 128)) return false;
	...
	if (FAILED(device->CreateTexture2D(&depthBufferDesc, NULL, &depthStencilBuffer))) return false;
	...
	if (FAILED(device->CreateDepthStencilState(&depthStencilDesc, &depthStencilState))) return false;
	...
	if (FAILED(device->CreateDepthStencilView(depthStencilBuffer,
		&depthStencilViewDesc, &depthStencilView))) return false;
...
void D3DFramework::BeginScene(float red, float green, float blue, float alpha)
{
...
void D3DFramework::EndScene()
{
	// Present the back buffer to the screen since rendering is complete.
	// Lock to screen refresh rate.
	if(isVsyncEnabled) swapChain->Present(1, 0);
	// Present as fast as possible.
	else swapChain->Present(0, 0);
}

ID3D11Device* D3DFramework::GetDevice()
{
	return device;
}

ID3D11DeviceContext* D3DFramework::GetDeviceContext()
{
	return deviceContext;
}

void D3DFramework::GetProjectionMatrix(D3DXMATRIX& projectionMatrix)
{
	projectionMatrix = this->projectionMatrix;
}


void D3DFramework::GetWorldMatrix(D3DXMATRIX& worldMatrix)
{
	worldMatrix = this->worldMatrix;
}


void D3DFramework::GetOrthoMatrix(D3DXMATRIX& orthoMatrix)
{
	orthoMatrix = this->orthoMatrix;
}


void D3DFramework::GetVideoCardInfo(char* cardName, int& memory)
{
