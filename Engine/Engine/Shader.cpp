...
bool Shader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (!InitializeShader(device, hwnd,
		L"../Engine/texture.vs", L"../Engine/texture.ps")) return false;
	return true;
}
								...
								D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	if (!SetShaderParameters(deviceContext, worldMatrix,
		viewMatrix, projectionMatrix, texture)) return false;
...
bool Shader::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFileName, const WCHAR* psFileName)
{
	...
	if (FAILED(device->CreateVertexShader(vertexShaderBuffer->GetBufferPointer(),
		vertexShaderBuffer->GetBufferSize(), NULL, &vertexShader))) return false;

    // Create the vertex shader from the buffer.
	if (FAILED(device->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
		pixelShaderBuffer->GetBufferSize(), NULL, &pixelShader))) return false;
	...
	if (FAILED(device->CreateInputLayout(polygonLayout,
		numElements, vertexShaderBuffer->GetBufferPointer(),
		vertexShaderBuffer->GetBufferSize(), &layout))) return false;
	...
	if (FAILED(device->CreateBuffer(&matrixBufferDesc,
		NULL, &matrixBuffer))) return false;
	...
	if (FAILED(device->CreateSamplerState(&samplerDesc, &sampleState))) return false;
											 ...
											 D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	if (FAILED(deviceContext->Map(matrixBuffer, 0,
